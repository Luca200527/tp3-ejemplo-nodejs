...
const getServicios = async (req, res) => {
    try {
        ...
        console.log(error)
       return res.status(500).json( { error: 'No se puedieron obtener los servicios' })
    }
}
...
const getServiciosById = async (req, res) => {
    try {
        ...
        console.log(error)
        return res.status(500).JSON( {error: `No se pudo obtener el datalle del servicio del id n° {id}` })
    }
}
...
module.exports = { getServicios, getServiciosById, getDetalle }